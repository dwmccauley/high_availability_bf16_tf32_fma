...
module cla_adder_8 (
    input  wire [7:0] a,
    input  wire [7:0] b,
    input  wire       cin,   // normally 0 for multiplier
    output wire [7:0] sum,
    output wire       cout
);
    ...
    // ToTry: Per Shibu Menon, 3-bit groupings are arguably more efficient than 4-bit groupings
...
//  always @*
//   	 $display("sum = %h, carry = %h, p = %h, g = %h", sum, carry, p, g);
